...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (susfs_is_inode_sus_path(link->dentry->d_inode)) {
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (!IS_ERR(dentry) && susfs_is_inode_sus_path(dentry->d_inode)) {
			   ...
			   struct path *path, struct path *root)
{
	...
	restore_nameidata();
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (!retval && susfs_is_inode_sus_path(path->dentry->d_inode)) {
		putname(name);
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (susfs_is_inode_sus_path(child->d_inode)) {
		int error = inode_permission(dir, MAY_WRITE | MAY_EXEC);
		if (error) {
			return error;
		}
		return -ENOENT;
