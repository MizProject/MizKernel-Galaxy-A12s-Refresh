...
static inline int may_follow_link(struct nameidata *nd)
{
	...
	if (nd->inode && unlikely(nd->inode->i_state ) && likely(current->susfs_task_state )) {
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (link->dentry->d_inode && unlikely(link->dentry->d_inode->i_state ) && likely(current->susfs_task_state)) {
		return -ENOENT;
	}
#endif
...
{
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (unlikely(inode->i_state ) && likely(current->susfs_task_state)) {
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (!IS_ERR(dentry) && dentry->d_inode && unlikely(dentry->d_inode->i_state ) && likely(current->susfs_task_state )) {
			   ...
			   struct path *path, struct path *root)
{
	...
	restore_nameidata();
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (!retval && path->dentry->d_inode && unlikely(path->dentry->d_inode->i_state ) && likely(current->susfs_task_state)) {
		putname(name);
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (child->d_inode && unlikely(child->d_inode->i_state ) && likely(current->susfs_task_state )) {
		error = inode_permission(dir, MAY_WRITE | MAY_EXEC);
		if (error) {
			return error;
		}
...
static int may_o_create(const struct path *dir, struct dentry *dentry, umode_t mode)
{
	...
	if (dentry->d_inode && unlikely(dentry->d_inode->i_state ) && likely(current->susfs_task_state )) {
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
		if (unlikely(dentry->d_inode->i_state ) && likely(current->susfs_task_state )) {
			dput(dentry);
			return -ENOENT;
		}
#endif
...
#ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
	if (!IS_ERR(filp) && unlikely(filp->f_inode->i_state) && current_uid().val < 2000) {
